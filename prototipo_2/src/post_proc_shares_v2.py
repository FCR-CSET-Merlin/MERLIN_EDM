...
for filepath in archivos_val:
    ...
    try:
        ...
        for sector, idx in idx_shares.items():
            ...
            # b) Forzar a 0 el share del sector actual
            X_val_puro[:, idx] = 1.0
            ...
            # predicciones_s_sector[sector] = pred_mwh
            
            # f) Guardar el sector en el dataframe local
            df_res_local[f'demanda_pred_{sector}'] = df_res_local["demanda_pred_mwh"] - pred_mwh
        # ==============================================================================
        # Pasar a dataframe el resultado
        # df_s_sector = pd.DataFrame(predicciones_s_sector)
...
if lista_resultados:
    ...
    ruta_guardado = os.path.join(results_dir, "resultados_modelo_nacional_desagregado_v2.parquet")
