...
def generate_temperature_lags(input_filepath, output_filepath, total_dimensions=8): 
    ...
    """
    Toma un .parquet de temperaturas en formato largo (fecha_hora, region, temperatura),
    ...
    df_long = df_long.sort_values(by=["region", "fecha_hora"]).reset_index(drop=True)
    
    # 3. Generar lags
    lags = total_dimensions - 1
    print(f"Generando {lags} horas de rezago para la temperatura agrupando por region...")
...
if __name__ == "__main__": 
    ...
    input_file = "../data/interim/temperatura_regional_horaria.parquet"
    output_file = "../data/interim/temperatura_regional_lagged.parquet"
