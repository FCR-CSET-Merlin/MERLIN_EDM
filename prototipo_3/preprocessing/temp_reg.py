...
def main():
    ...
    for year in range(2017, 2027):
        ...
        for month in meses:
            ...
            for idx, row in gdf_regiones.iterrows():
                ...
                # Buscar el límite urbano correspondiente a esta region
                ...
                if temp_region_series is None:
                    # 1. Definir qué polígono usar (priorizar urbano, luego regional)
                    ...
                    if poligono_base is None or poligono_base.is_empty:
                        print(f"   [!] Error espacial: La región {nombre_region} no tiene geometría válida. Se omitirá.")
                    ...
                    if centroide.is_empty:
                        print(f"   [!] Error espacial: Centroide vacío para {nombre_region}. Se omitirá.")
                        continue

                    # 5. Extraer temperatura con Nearest Neighbor
                    temp_region_series = da_temp.sel(
    ...
    ruta_salida = os.path.join(OUT_DIR, "temperatura_regional_horaria.parquet")
    df_final.to_parquet(ruta_salida, engine='pyarrow', index=False)
